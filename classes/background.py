...
class Background():
    ...
    def __init__(self, mediator, screen):
        ...
        for floor_num in range(1, 10):
            ...
            if os.path.exists(floor_image_path):
                self.elevator_floor_ground_surfs[floor_num] = helpers.load_image(
                    os.path.join(
                        os.path.dirname(os.path.dirname(__file__)),
                        "media",
                        "graphics",
                        "hotel",
                        "indoors_ground_large.png"))
                self.elevator_floor_sky_surfs[floor_num] = helpers.load_image(floor_image_path)
        ...
        for floor_num in range(1, 10):
            ...
            if os.path.exists(floor_open_image_path):
                self.elevator_open_floor_ground_surfs[floor_num] = helpers.load_image(
                    os.path.join(
                        os.path.dirname(os.path.dirname(__file__)),
                        "media",
                        "graphics",
                        "hotel",
                        "indoors_ground_large.png"))
                self.elevator_open_floor_sky_surfs[floor_num] = helpers.load_image(floor_open_image_path)
        
        # Corridor backgrounds for floors 3-9 (corridors 1, 3, 4)
        self.corridor_ground_surfs = {}  # {(floor_num, corridor_num): surface}
        self.corridor_sky_surfs = {}      # {(floor_num, corridor_num): surface}
        corridor_numbers = [1, 3, 4]  # Corridors that exist
        for floor_num in range(3, 10):
            for corridor_num in corridor_numbers:
                ...
                if os.path.exists(corridor_image_path):
                    self.corridor_ground_surfs[(floor_num, corridor_num)] = helpers.load_image(
                        os.path.join(
                            os.path.dirname(os.path.dirname(__file__)),
                            "media",
                            "graphics",
                            "hotel",
                            "indoors_ground_large.png"))
                    self.corridor_sky_surfs[(floor_num, corridor_num)] = helpers.load_image(corridor_image_path)
        
        # Track current floor and corridor for navigation
        self.current_floor = None  # Current floor number (3-9)
        self.current_corridor = None  # Current corridor number (1, 3, 4, or None for elevator)
    ...
    def switch_to_entrance(self):
        ...
        Changes the current background from yard, frontdesk, or elevator_lobby to entrance if currently
        at yard, frontdesk, or elevator_lobby. Updates ground and sky surfaces accordingly and notifies
        the mediator of the change.
        """
        if self.current_background == "yard" or self.current_background == "frontdesk" or self.current_background == "elevator_lobby":
    ...
    def switch_to_frontdesk(self):
        ...
        Changes the current background from entrance or elevator_lobby to frontdesk if currently
        at entrance or elevator_lobby. Updates ground and sky surfaces accordingly and notifies
        the mediator of the change.
        """
        if self.current_background == "entrance" or self.current_background == "elevator_lobby":
            ...
            self.notify('Background switched to frontdesk.')

    def switch_to_elevator_lobby(self):
        """
        Switch the background to the elevator lobby scene.
        
        Changes the current background from frontdesk or any elevator_open background 
        to elevator lobby if currently at frontdesk or any elevator_open background. 
        Updates ground and sky surfaces accordingly and notifies the mediator of the change.
        """
        ...
        if self.current_background == "frontdesk" or is_elevator_open:
            ...
            self.notify('Background switched to elevator lobby.')

    def switch_to_elevator_open_lobby(self):
        """
        Switch the background to the elevator open lobby scene.
        
        Changes the current background from elevator_lobby to elevator_open_lobby if currently
        at elevator_lobby. Updates ground and sky surfaces accordingly and notifies
        the mediator of the change.
        """
        if self.current_background == "elevator_lobby":
            ...
            self.notify('Background switched to elevator open lobby.')

    def switch_to_elevator_floor(self, floor_num):
        """
        Switch the background to the specified elevator floor scene.
        
        Changes the current background from any elevator_open background or corridor
        to elevator_floor{floor_num} if the floor background exists.
        Updates ground and sky surfaces accordingly and notifies the mediator of the change.
        ...
        is_elevator_open = (self.current_background == "elevator_open_lobby" or
                           self.current_background.startswith("elevator_open_floor"))
        is_corridor_on_same_floor = (self.current_background.startswith("corridor") and
        ...
        if ((is_elevator_open or is_corridor_on_same_floor) and 
            ...
            self.notify(f'Background switched to elevator floor {floor_num}.')
    
    def switch_to_elevator_open_floor(self, floor_num):
        """
        Switch the background to the specified elevator open floor scene.
        
        Changes the current background from elevator_floor{floor_num} to elevator_open_floor{floor_num}
        if currently at that floor and the open floor background exists.
        Updates ground and sky surfaces accordingly and notifies the mediator of the change.
        ...
        if (self.current_background == f"elevator_floor{floor_num}" and 
            ...
            self.current_floor = floor_num
            self.current_corridor = None  # At elevator, not in corridor
            self.notify(f'Background switched to elevator open floor {floor_num}.')
    
    def switch_to_corridor(self, floor_num, corridor_num):
        """
        Switch the background to the specified corridor scene.
        
        Changes the current background to corridor{corridor_num} on floor{floor_num}
        if the corridor background exists.
        Updates ground and sky surfaces accordingly and notifies the mediator of the change.
        ...
        if (floor_num, corridor_num) in self.corridor_ground_surfs and \
           (floor_num, corridor_num) in self.corridor_sky_surfs:
            ...
            self.notify(f'Background switched to corridor {corridor_num} on floor {floor_num}.')
